...
typedef struct Node{
    ...
    int step; // s? b??c di chuy?n ð? ði t? tr?ng thái xu?t phát ð?n tr?ng thái hi?n t?i
    struct Node* next; // con tr? ð?n ph?n t? ti?p theo trong hàng ð?i
    struct Node* parent;// con tr? tr? ð?n tr?ng thái sinh ra tr?ng thái hi?n t?i
}Node;
...
// CODE thua, khong can phai dung den bo nho phu nay!
Node* listNode[MAX*MAX];// m?ng l?u các ph?n t? ð??c c?p phát ð?ng, ð? gi?i phóng BN
int szList = 0;// s? ph?n t? c?a listNode
